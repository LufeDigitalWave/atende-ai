...
TEMPLATE_FAQ_RAG_PATH = Path(__file__).parent.parent / "agent" / "prompts" / "agent_template_faq_rag.md"
...
def render_prompt(profile: NicheProfile, agent_type: str = "sdr") -> str:
    """Render the prompt template with the NicheProfile data.

    Args:
        profile: The NicheProfile generated by Factory.
        agent_type: 'sdr' uses the full SDR template; 'faq_rag' uses the FAQ/RAG template.

    Returns the final system prompt string. Safe to ship to the LLM.
    """
    template_path = TEMPLATE_FAQ_RAG_PATH if agent_type == "faq_rag" else TEMPLATE_PATH
